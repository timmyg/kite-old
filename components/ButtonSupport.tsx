...
const ButtonSupport = () => {
  const handleClick = () => {
    if (platformConfig.crisp?.id) {
      Crisp.chat.show();
      Crisp.chat.open();
    }
    // else if (platformConfig.mailgun?.supportEmail) {
    //   // open default email client in new window with "need help with ${platformConfig.appName}" as subject
    //   window.open(
    //     `mailto:${platformConfig.mailgun.supportEmail}?subject=Need help`,
    //     "_blank"
    //   );
    // }
  };
